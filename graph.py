...
class GraphDisplay:
	...
	def FreshValue(self, variable , value , color):
		if not self.playing:
			return 0
		try:
                        if variable != "Linea" and variable != "Angulo":
                                return 0
	...
	def GetRelPos(self,x):
                value = self.reference[self.variables[x]].circle_y
                if value > 10000000:
                        value = 1000000
                if value < -10000000:
                        value = -1000000
...
class SelectionWindow(BasicApp):
	...
	def GetPorts(self):
		...
		for p in ports:
			if p[2] != 'n/a':
				real.append({"status":"dead","data":p,"baudrate":115200,"last_time":0,"button":Selector(),"bnumber":7})
